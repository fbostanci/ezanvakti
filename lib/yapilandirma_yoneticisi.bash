...
function yapilandirma() {
  ...
  local _GUNCELLEME_YAP _OYNATICI_DURAKLAT _EZAN_DUASI_OKU _RENK_KULLAN
...
"$d4" "$d5" "$d6" "$d7" "$d8" \
"$GUN_ANIMSAT[!0..30[!1]]" "$SES[!0..100[!1]]" \
...
--button='gtk-save:174' --button='gtk-quit:121'
 ...
 case $? in
    ...
    174)
            ...
            if [[ ${GUN_ANIMSAT} != ${liste3[5]} ]]
            then
                sed -i "s:\(GUN_ANIMSAT=\).*:\1\'${liste3[5]}\':" "${EZANVAKTI_AYAR}"
            fi

            if [[ ${EZAN_BILDIRIM_SURESI} != ${liste3[6]} ]]
            then
                sed -i "s:\(EZAN_BILDIRIM_SURESI=\).*:\1\'${liste3[6]}\':" "${EZANVAKTI_AYAR}"
            fi

            if [[ ${AYET_BILDIRIM_SURESI} != ${liste3[7]} ]]
            then
                sed -i "s:\(AYET_BILDIRIM_SURESI=\).*:\1\'${liste3[7]}\':" "${EZANVAKTI_AYAR}"
            fi

            if [[ ${HADIS_BILDIRIM_SURESI} != ${liste3[8]} ]]
            then
                sed -i "s:\(HADIS_BILDIRIM_SURESI=\).*:\1\'${liste3[8]}\':" "${EZANVAKTI_AYAR}"
            fi

            if [[ ${BILGI_BILDIRIM_SURESI} != ${liste3[9]} ]]
            then
                sed -i "s:\(BILGI_BILDIRIM_SURESI=\).*:\1\'${liste3[9]}\':" "${EZANVAKTI_AYAR}"
            fi

            if [[ ${GUNCELLEME_BILDIRIM_SURESI} != ${liste3[10]} ]]
            then
                sed -i "s:\(GUNCELLEME_BILDIRIM_SURESI=\).*:\1\'${liste3[10]}\':" "${EZANVAKTI_AYAR}"
            fi
...
#             if [[ ${liste4[1]} != ${RENK1} ]]
#             then
#                 sed -i "s:\(RENK1=\).*:\1\'${liste4[1]}\':" "${EZANVAKTI_AYAR}"
#             fi
#
#             if [[ ${liste4[2]} != ${RENK2} ]]
#             then
#                 sed -i "s:\(RENK2=\).*:\1\'${liste4[2]}\':" "${EZANVAKTI_AYAR}"
#             fi
#
#             if [[ ${liste4[3]} != ${RENK3} ]]
#             then
...
#             fi
#
            if [[ ${liste4[9]} != ${ARKAPLAN_RENGI} ]]
            then
                sed -i "s:\(ARKAPLAN_RENGI=\).*:\1\'${liste4[9]}\':" "${EZANVAKTI_AYAR}"
            fi

            if [[ ${liste4[10]} != ${YAZI_RENGI} ]]
            then
                sed -i "s:\(YAZI_RENGI=\).*:\1\'${liste4[10]}\':" "${EZANVAKTI_AYAR}"
            fi
